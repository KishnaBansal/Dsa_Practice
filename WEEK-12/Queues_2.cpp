...
void firstNegativeInt(int *arr,int n,int k) {
    ...
    //Previous loop ends at (k-1)th index. So the next loop will start from k index 
    for(int i=k;i<n;i++) {
        //aage badhne se pehle print -ve elements of old window
        if(dq.empty()) {
        cout<<0<<" ";
        }
        else {
        ...
        //Removal
        //Jo bhi index out of range hai use queue se remove kardo
        if(i - dq.front() >= k) {
            dq.pop_front();
        }
        //Addition
